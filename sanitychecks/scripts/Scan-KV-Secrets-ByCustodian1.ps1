...
function Ensure-Dir([string]$p){
  if([string]::IsNullOrWhiteSpace($p)){ $p = Join-Path (Get-Location) 'kv-secrets-out' }
...
$outCsv = Join-Path $OutputDir "kv_secrets_ByCustodian_$stamp.csv"
$outHtml= Join-Path $OutputDir "kv_secrets_ByCustodian_$stamp.html"
$outJson= Join-Path $OutputDir "kv_secrets_ByCustodian_$stamp.json"
    ...
    foreach($v in $vaults){
      foreach($name in $exp){
        ...
        $rows.Add([pscustomobject]@{
          Custodian=$cust; SubscriptionName=$s.Name; SubscriptionId=$s.Id
...
($rows | ConvertTo-Html -Title "KV Secrets ByCustodian $stamp" -PreContent "<h2>KV Secrets ByCustodian ($BranchName)</h2>") | Set-Content -Path $outHtml -Encoding UTF8
